...
assessmentRouter.post('/', async (req: Request, res: Response) => {
  const { room_url, sessionId, case_name } = req.body;

  if (!room_url || !sessionId || !case_name) {
    console.log('Missing room_url, sessionId or case_name');
    return res.status(400).json({ error: 'Missing room_url, sessionId or case_name' });
  }
  ...
  try {
    const assessment = await runAssessment(case_name, roomName, sessionId);
